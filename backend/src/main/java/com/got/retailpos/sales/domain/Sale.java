...
public class Sale {
	...
	public void awaitPayment(String key){if(status!=SaleStatus.DRAFT)throw new IllegalStateException("เริ่มชำระบิลนี้ไม่ได้");status=SaleStatus.AWAITING_PAYMENT;checkoutIdempotencyKey=key;updatedAt=Instant.now();}
	public void completePromptPay(String receipt,UUID actor){if(status!=SaleStatus.AWAITING_PAYMENT&&status!=SaleStatus.EXPIRED)throw new IllegalStateException("ยืนยัน PromptPay ของบิลนี้ไม่ได้");status=SaleStatus.COMPLETED;receiptNumber=receipt;completedBy=actor;completedAt=Instant.now();updatedAt=completedAt;}
	public void expirePayment(){if(status!=SaleStatus.AWAITING_PAYMENT)throw new IllegalStateException("หมดอายุบิลนี้ไม่ได้");status=SaleStatus.EXPIRED;checkoutIdempotencyKey=null;updatedAt=Instant.now();}
